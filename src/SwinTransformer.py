...
class ConvPatchEmbed4x(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        # Custom CNN based patch embedding as described in the paper
        def conv_layer(input_nc, output_nc, kernel_size=3, stride=2, padding=1):
            return nn.Sequential(
                nn.Conv2d(input_nc, output_nc, kernel_size, stride, padding, bias=False),
                nn.ReLU(inplace=True)
            )

        self.convs = nn.Sequential(
            conv_layer(in_chans, embed_dim // 2, 3, 2, 1),
            conv_layer(embed_dim // 2, embed_dim // 2, 3, 1, 1),
            conv_layer(embed_dim // 2, embed_dim, 3, 2, 1),
            conv_layer(embed_dim, embed_dim, 3, 1, 1),
...
class SwinTransformer(nn.Module):
    def __init__(self, pretrain_img_size=224, patch_size=4, in_chans=3, embed_dim=96,
        ...
        self.patch_embed = ConvPatchEmbed4x(patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim)
